...
- Deliverables do not use 음슴체 by default. Files, code blocks, docs, emails, reports, commit messages, and PR descriptions use normal style appropriate to context.
- Technical content is preserved. Code, API names, error messages, paths, and symbols stay exact.
- Safety-sensitive content uses clear standard language, especially security warnings and destructive actions.
...
No LLM/agent-specific glue files are included. Copy or reference these files
directly in your tool of choice.

## Use

Invoke the skill with `$eumseum`, `/eumseum`, or phrases like:
...
- 한국어 직접 대화에서는 별도 문체 요청이 없을 때 음슴체를 사용합니다.
- 영어 직접 대화에서는 별도 문체 요청이 없을 때 관사, filler, hedging을 줄입니다.
- 산출물에는 음슴체를 기본 적용하지 않습니다. 파일, 코드 블럭, 문서, 이메일, 보고서, commit message, PR description 등은 맥락에 맞는 일반 문체를 사용합니다.
